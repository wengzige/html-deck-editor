...
export function detectDeck(input: LoadedInput): DetectionReport {
  ...
  if (isDocumentLikePage(doc)) {
    return {
      ...
      slideCount: 1,
      confidence: 0.35,
      messages: ["这更像纵向文档/报告页面，不是标准幻灯片。"],
      warnings: [
        "这份 HTML 更像纵向文档或报告；本地转换会按 1 页保留原布局。",
        "如果选择 AI 智能适配，AI 会尝试把文档重组为演示稿，但页数、顺序和布局都需要人工复核。",
        ...aiWarnings
      ]
    };
  }
...
function isDocumentLikePage(doc: Document): boolean {
  const topSections = Array.from(doc.body.querySelectorAll("main > section, body > section")).filter(hasEnoughText);
  if (topSections.length < 3) return false;

  const bodyText = compactWhitespace(doc.body.textContent || "");
  if (bodyText.length < 160) return false;

  const signalText = compactWhitespace([
    doc.title,
    doc.querySelector("h1, h2, main > section:first-child")?.textContent || "",
    bodyText.slice(0, 2400)
  ].join(" "));
  const termMatched = /(报告|報告|检测|檢測|评估|評估|审计|審計|分析|身份驗證|身份验证|信心度|置信度|confidence|benchmark|scorecard|audit|assessment|findings|appendix|report|review)/i.test(signalText);
  const constrained = hasConstrainedBodyLayout(doc);
  const semanticDocument = hasDocumentSemantics(doc);
  const explicitPresentation = hasPresentationSemantics(doc);
  const textBlocks = doc.body.querySelectorAll("p, li, table, details, blockquote, pre").length;
  const headings = doc.body.querySelectorAll("h1, h2, h3, h4").length;
...
function hasDocumentSemantics(doc: Document): boolean {
...
function hasPresentationSemantics(doc: Document): boolean {
  const presentation = doc.body.querySelector(explicitDeckParentSelector);
  if (!presentation) return false;
  const identity = [
    presentation.id,
    presentation.className,
    presentation.getAttribute("role") || "",
    presentation.getAttribute("aria-label") || ""
  ].join(" ");
  return !/(article|content|document|markdown|post|prose|report|audit)/i.test(identity);
}
