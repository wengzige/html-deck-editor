...
type ValidatedElementSelector = {
  selector: string;
  element: Element;
};

type ValidatedSlideCandidate = {
  selector: string;
  title: string;
  element: Element;
};
    ...
    messages: [
      ...
      },
      {
        ...
        content: [
          ...
          "stageSelector 如果填写，不能同时出现在 slides 里，也不能指向任何 slide 内部的元素。",
          "",
          summary
        ].join("\n")
      }
...
export function validateAiAdaptationPlan(doc: Document, plan: AiAdaptationPlan): ValidatedAiAdaptationPlan {
  ...
  const stageCandidate = firstValidElementSelector(doc, plan.stageSelector, warnings, "舞台容器");
  const slides = topLevelSlideCandidates(
    dropStageContainerFromSlides(validSlides(doc, plan.slides || [], warnings), stageCandidate, warnings),
    warnings
  );

  if (slides.length < 1) {
    throw new Error("AI 适配计划没有识别出至少 1 页。");
  }

  return {
    stageSelector: validStageSelector(stageCandidate, slides, warnings),
...
function validSlides(doc: Document, slides: AiAdaptationSlide[], warnings: string[]): ValidatedSlideCandidate[] {
  const seen = new Set<Element>();
  const valid: ValidatedSlideCandidate[] = [];
...
function dropStageContainerFromSlides(
  slides: ValidatedSlideCandidate[],
  stageCandidate: ValidatedElementSelector | null,
  warnings: string[]
): ValidatedSlideCandidate[] {
...
function topLevelSlideCandidates(slides: ValidatedSlideCandidate[], warnings: string[]): ValidatedSlideCandidate[] {
  const topLevel = slides.filter((slide) => !slides.some((other) => other !== slide && other.element.contains(slide.element)));
  if (topLevel.length !== slides.length) {
    warnings.push("AI 适配计划包含嵌套页面 selector，已只保留最外层页面。");
  }
  return topLevel;
}

function validStageSelector(
  candidate: ValidatedElementSelector | null,
  slides: ValidatedSlideCandidate[],
  warnings: string[]
): string | null {
  if (!candidate) return null;
  const conflictsWithSlide = slides.some((slide) => {
    return candidate.element === slide.element || slide.element.contains(candidate.element);
  });
  if (conflictsWithSlide) {
...
function firstValidElementSelector(doc: Document, selector: unknown, warnings: string[], label: string): ValidatedElementSelector | null {
...
function isDocumentContainer(element: Element): boolean {
