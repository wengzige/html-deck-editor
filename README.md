...
> 当前在线工作台面向横屏 PC，支持导入 HTML / HTM、ZIP 压缩包和完整文件夹。修改需要主动保存；暂不提供实时自动保存、历史版本或分享链接。

**[打开在线工作台 →](https://anchordeck.dpdns.org/)**

## 也可以选择本地优先

在线工作台不是唯一入口。希望文件默认只在浏览器本地处理，或希望让当前 AI 客户端直接修改本机文件时，可以选择下面两条路线：
...
| --- | --- | --- | --- | --- |
| **在线工作台** | 直接导入 HTML / ZIP / 文件夹，并在账户项目中继续处理 | 浏览器与账户项目 | 平台 AI 或自己的 API | 使用平台 AI 时不需要 |
...
[结构契约](docs/html-deck-contract.md) · [隐私说明](docs/privacy.md) · [GitHub Releases](https://github.com/wengzige/html-deck-editor/releases/latest)
